...
- [`FileSource`]: Loads configuration from a file, detecting `.toml`, `.json`, or `.ron` files based on the file extension. Requires the matching `toml`, `json`, or `ron-0_12` feature. (`toml` is enabled by default.)
