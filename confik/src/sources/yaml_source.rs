...
#[derive(Clone)]
pub struct YamlSource<'a> {
    contents: Cow<'a, str>,
    allow_secrets: bool,
}

impl<'a> YamlSource<'a> {
    /// Creates a [`Source`] containing raw YAML data.
...
impl<T: ConfigurationBuilder> Source<T> for YamlSource<'_> {
    fn allows_secrets(&self) -> bool {
        self.allow_secrets
    }

    fn provide(&self) -> Result<T, Box<dyn Error + Sync + Send>> {
        Ok(yaml_serde_0_10::from_str(&self.contents)?)
    }
}

impl fmt::Debug for YamlSource<'_> {
...
mod tests {
    ...
    #[test]
    fn provides_yaml_data() {
        let source = YamlSource::new("value: 42\n");

        let config =
            <YamlSource<'_> as Source<<TestConfig as crate::Configuration>::Builder>>::provide(
                &source,
            )
            .unwrap()
            .try_build()
    ...
    fn propagates_parse_errors() {
        let source = YamlSource::new("value: [\n");

        let err =
            match <YamlSource<'_> as Source<<TestConfig as crate::Configuration>::Builder>>::provide(
                &source,
            ) {
                Ok(_) => panic!("YAML parsing should fail"),
                Err(err) => err,
            };
